...
class Permissions:
	...
	async def setpermission(self):
		'''Set a permission'''
		await self.bot.embed_reply(":no_entry: Invalid input. setpermission everyone|role|user")
	...
	async def setpermission_everyone(self, ctx, permission : str, setting : bool = None):
		...
		await self.bot.embed_reply("Permission updated. {} set to {} for everyone".format(permission, setting))
	...
	async def setpermission_role(self, ctx, role : str, permission : str, setting : bool = None):
		...
		with open("data/permissions/{}.json".format(ctx.message.server.id), "w") as permissions_file:
			json.dump(permissions_data, permissions_file, indent = 4)		
		await self.bot.embed_reply("Permission updated. {} set to {} for the {} role".format(permission, setting, _role.name))
	...
	async def setpermission_user(self, ctx, user : str, permission : str, setting : bool = None):
		...
		with open("data/permissions/{}.json".format(ctx.message.server.id), "w") as permissions_file:
			json.dump(permissions_data, permissions_file, indent = 4)
		await self.bot.embed_reply("Permission updated. {} set to {} for {}".format(permission, setting, _user))
	...
	async def getpermission(self, ctx, *options : str):
		...
		if len(options) == 2:
			...
			await self.bot.embed_reply(":no_entry: Invalid input. getpermission everyone|role|user or <user> <permission>") #options
	...
	async def getpermissions(self):
		await self.bot.embed_reply(":no_entry: Invalid input. getpermissions everyone|role|user|command")
