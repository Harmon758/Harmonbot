...
class AudioPlayer:
    ...
    async def player_task(self):
        ...
        while True:
            ...
            if not source.stream:
                now_playing_message = await self.bot.send_embed(
                    self.text_channel, ":arrow_down: Downloading..",
                    title = source.info.get("title", "N/A"),
                    title_url = source.info.get("webpage_url"),
                    timestamp = source.timestamp,
                    footer_text = source.requester.display_name,
                    footer_icon_url = source.requester.avatar.url,
                    thumbnail_url = source.info.get("thumbnail")
                )
            ...
            if source.stream:
                await self.bot.send_embed(
                    self.text_channel, ":arrow_forward: Now Playing",
                    title = source.info.get("title", "N/A"),
                    title_url = source.info.get("webpage_url"),
                    timestamp = source.timestamp,
                    footer_text = source.requester.display_name,
    ...
    async def play_file(self, ctx, filename):
        ...
        return await self.interrupt(
            FileSource(
                ctx, ctx.bot.data_path + "/audio_files/" + filename,
                self.default_volume, title_prefix = "Audio File: "
            )
        )
    ...
    async def play_from_library(
        ...
        if not filename and self.library_files:
            ...
            await ctx.embed_reply(f"{ctx.bot.error_emoji} Song file not found")
            return True
        return await self.interrupt(
            FileSource(
