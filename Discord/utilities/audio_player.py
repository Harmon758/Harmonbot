...
class AudioPlayer:
	...
	def __init__(self, bot, text_channel):
		...
		self.library_files = [f for f in os.listdir(clients.library_path) if f.endswith((".mp3", ".m4a"))]
	...
	async def play_from_library(self, filename, requester, timestamp, *, clear_flag = True):
		if not filename:
			...
			return True
		return (await self._interrupt(clients.library_path + filename, filename, requester, timestamp, clear_flag = clear_flag))
		## print([f for f in os.listdir(clients.library_path) if not f.endswith((".mp3", ".m4a", ".jpg"))])
