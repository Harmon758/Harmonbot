...
MINERALS = {  # https://en.wikipedia.org/wiki/Mohs_scale_of_mineral_hardness
...
WOOD_TYPES = [  # https://en.wikipedia.org/wiki/Janka_hardness_test
...
class Adventure(commands.Cog):
    ...
    async def forage(self, ctx, *, item: str = ""):
        ...
        if started is True:
            await ctx.embed_reply(
                f"\N{HERB} You have started foraging for {item}"
            )
            # TODO: active option?
    ...
    async def forage_start(self, ctx, *, item: str):
        ...
        if started is True:
            ...
            await ctx.embed_reply(
                f"{ctx.bot.error_emoji} You're currently {started}! "
                "You can't start foraging right now"
            )
    ...
    async def stats_woodcutting(self, ctx):
        ...
        await ctx.embed_reply(
            f"\N{EVERGREEN TREE} Woodcutting xp: {woodcutting_xp:,}\n"
    ...
    async def woodcutting_start(self, ctx, *, wood_type: str):
        ...
        if started is True:
            ...
            await ctx.embed_reply(
                f"{ctx.bot.error_emoji} You're currently {started}! "
                "You can't start woodcutting right now"
            )
    ...
    async def woodcutting_stop(self, ctx):
        ...
        if stopped[0]:
            ...
            await ctx.embed_reply(f"{ctx.bot.error_emoji} You're currently {stopped[1]}! You aren't woodcutting right now")
        else:
            await ctx.embed_reply(f"{ctx.bot.error_emoji} You aren't woodcutting")
