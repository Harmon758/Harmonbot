...
class Respects(commands.Cog):
	...
	async def stats(self, ctx):
		...
		axes = figure.add_subplot(
			xscale = "log", xlabel = "Respects Paid", ylabel = "People"
		)
		last_power_of_10 = math.ceil(numpy.log10(max(respects_paid)))
		bins = (
			(10 ** numpy.arange(last_power_of_10))[:, numpy.newaxis] *
			numpy.arange(1, 10)
		)
		axes.hist(
			respects_paid, bins = bins.flatten(), log = True, ec = "black"
		)
		## bin_centers = bins[:-1] + numpy.diff(bins) / 2
		## def func(x, a, b, c):
		##	return a * numpy.exp(-b * x) + c
		## popt, _ = scipy.optimize.curve_fit(
		## 	func, bin_centers[1:], n[1:], p0 = (1000, 1, 1), maxfev = 100000
		## )
		...
		## 	x_interval_for_fit, func(x_interval_for_fit, *popt), color = "red"
		## )
		## axes.set_ylim(0.8)
		...
		axes.set_xticks(
			[tick for tick in axes.get_xticks(minor = True) if tick >= 1],
			minor = True
		)
		axes.set_yticks(
			[tick for tick in axes.get_yticks(minor = True) if tick >= 1],
			minor = True
		)
		...
		await ctx.embed_reply(
			fields = (
				("Total respects paid", f"{total_respects:,}"),
