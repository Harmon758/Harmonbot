...
class Meta(commands.Cog):
    ...
    async def cog(self, ctx, command):
        ...
        if command not in self.bot.all_commands:
            await ctx.embed_reply(
                f"{ctx.bot.error_emoji} Error: command not found"
            )
            return
    ...
    async def version_ffmpeg(self, ctx):
        ...
        output = subprocess.run(  # noqa: S603 (subprocess-without-shell-equals-true)
            f"{ctx.bot.bin_path}ffmpeg -version",
    ...
    async def version_library(self, ctx, library: str):
        ...
        try:
            ...
            await ctx.embed_reply(
                f"{ctx.bot.error_emoji} Error: {library} library not found"
            )
    ...
    @commands.guild_only()
    @checks.not_forbidden()
    async def harmonbot_activity(self, ctx, *, name: Optional[str]):  # noqa: UP045 (non-pep604-annotation-optional)
        '''My activity'''
        # TODO: Handle in DMs
    ...
    @commands.guild_only()
    @checks.not_forbidden()
    async def harmonbot_activity_type(self, ctx, type: Optional[str]):  # noqa: UP045 (non-pep604-annotation-optional)
        '''
        My activity type
    ...
    @commands.guild_only()
    @checks.not_forbidden()
    async def harmonbot_activity_url(self, ctx, url: Optional[str]):  # noqa: UP045 (non-pep604-annotation-optional)
        '''My activity url'''
        activity = ctx.me.activity
        if not url:
