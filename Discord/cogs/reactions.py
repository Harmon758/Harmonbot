	...
	async def playingr_processr(self, ctx, reaction, user):
		if reaction.emoji in self.controls:
			if self.controls[reaction.emoji] == "pause_resume":
				if clients.get_permission(ctx, "pause", id = user.id) or user == ctx.guild.owner or user.id == self.bot.owner_id:
				...
				if clients.get_permission(ctx, self.controls[reaction.emoji], id = user.id) or user.id in (ctx.guild.owner.id, self.bot.owner_id):
				...
				if clients.get_permission(ctx, "volume", id = user.id) or user.id in (ctx.guild.owner, self.bot.owner_id):
					try:
