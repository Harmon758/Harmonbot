...
class Bot(commands.Bot):
	...
	async def on_guild_join(self, guild):
		...
		await self.send_embed(me, title = "Joined Server", timestamp = guild.created_at, thumbnail_url = guild.icon_url, fields = (("Name", guild.name), ("ID", guild.id), ("Owner", str(guild.owner)), ("Members", str(guild.member_count)), ("Server Region", str(guild.region))))
		# TODO: Track guild names
	
	async def on_guild_remove(self, guild):
		self.loop.create_task(self.update_all_listing_stats())
		me = discord.utils.get(self.get_all_members(), id = self.owner_id)
		await self.send_embed(me, title = "Left Server", timestamp = guild.created_at, thumbnail_url = guild.icon_url, fields = (("Name", guild.name), ("ID", guild.id), ("Owner", str(guild.owner)), ("Members", str(guild.member_count)), ("Server Region", str(guild.region))))
