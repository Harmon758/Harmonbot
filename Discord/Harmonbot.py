	...
	async def on_message(message):
		...
		if channel.type is discord.ChannelType.private:
			try:
				...
				# Forbidden from fetching channel here means this is a user
				# integration interaction in a guild and this is an ephemeral
				# message that is an instance of the below issue.
				ctx.bot.print(
				...
				# even if it was not sent as a DM. This is an issue with
				# discord.py caused by Discord's API not providing guild_id for
				# ephemeral messages for the MESSAGE_CREATE event. This might
				# eventually be fixed by Discord.
				...
				if channel.type is not discord.ChannelType.private:
					...
					forwarded_message = await message.forward(ctx.bot.owner)
	...
	async def main():
		async with client:
			...
			if ci or client.beta:
				...
				opus = discord.opus.libopus_loader(f"{client.bin_path}opus.dll")
