		...
		if channel.type is discord.ChannelType.private:
			if not channel.recipient:
				try:
					...
					# integration interaction in a guild and this is an
					# ephemeral message that is an instance of the below issue.
					ctx.bot.print(
						f"Ephemeral message with erroneous DMChannel channel attribute: {message.id}"
					)
			# message.channel is a DMChannel here for an ephemeral message even
			# if it was not sent as a DM. This is an issue with discord.py
			# caused by Discord's API not providing guild_id for ephemeral
			# messages for the MESSAGE_CREATE event. This might eventually be
			# fixed by Discord.
