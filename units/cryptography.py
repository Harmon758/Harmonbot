...
def encode_caesar_cipher(message: str, key: int):
    ...
    for character in message:
        if not character.isalpha() or not character.isascii():
            ...
            encoded_message += ascii_lowercase[
                (ascii_lowercase.index(character) + key) % 26
            ]
        else:
            encoded_message += ascii_uppercase[
                (ascii_uppercase.index(character) + key) % 26
            ]
